...
import { ArrowUpRight, ChevronDown, ChevronUp, ShieldCheck, CheckCircle2, Target, ArrowRight } from "lucide-react";
import { PremiumUpgradeModal } from "@/components/home/PremiumUpgradeModal";

interface HeroSectionProps {
  onStartScan: (targetOverride?: string) => void;
}
...
export function HeroSection({ onStartScan }: HeroSectionProps) {
  ...
  const handleToggleScanOptions = () => {
    setShowScanOptions((value) => !value);
  };
  ...
  const scanConfigModal = showScanOptions
    ? createPortal(
        <div
          ref={backdropRef}
          className="fixed inset-0 z-[9998] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4 py-6"
          onClick={(event) => {
        ...
        >
          <div
            className="relative z-[9999] w-full max-w-xl overflow-hidden rounded-[28px] border border-slate-800 bg-slate-950/98 shadow-[0_35px_120px_-30px_rgba(0,0,0,0.7)] animate-in fade-in slide-in-from-bottom-2 duration-200"
            role="dialog"
            aria-modal="true"
          ...
          >
            <div className="max-h-[90vh] overflow-y-auto p-5 sm:p-6">
              <div className="space-y-4 px-5 pb-5 pt-4">
                ...
                <div>
                  <label className="text-sm text-slate-300">Scan Type</label>
                  <select className="mt-2 w-full rounded-2xl bg-white/5 border border-white/10 px-4 py-3 text-white">
                ...
                <div>
                  <label className="text-sm text-slate-300">Authentication Type</label>
                  <input
                    type="text"
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <div>
                    <label className="text-sm text-slate-300">Crawl Depth</label>
                    <input
                      type="number"
                      placeholder="5"
                      className="mt-2 w-full rounded-2xl bg-white/5 border border-white/10 px-4 py-3 text-white outline-none"
                  ...
                  <div>
                    <label className="text-sm text-slate-300">File Extensions to Exclude</label>
                    <input
                      type="text"
                ...
                <div>
                  <label className="text-sm text-slate-300">Exclude URLs</label>
                  <textarea
                    placeholder="Enter URLs to exclude"
                    className="mt-2 w-full rounded-2xl bg-white/5 border border-white/10 px-4 py-3 text-white outline-none resize-none"
                  />
                </div>
                ...
                <div className="flex flex-col gap-3 sm:flex-row">
                  <button
                    ...
                    className="flex-1 rounded-2xl border border-slate-700 bg-slate-900/80 px-4 py-3 text-sm font-semibold text-slate-200 hover:bg-slate-800 transition-colors"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-2xl bg-purple-600 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-purple-500/20 hover:bg-purple-500 transition-colors"
                  >
                    Start Scan
                  </button>
                </div>
  ...
  return (
    <div className="w-full flex flex-col items-center text-center pt-6 pb-12">
      ...
      <div className="mt-8 w-full max-w-xl mx-auto relative overflow-visible">
        <div className="rounded-2xl p-1.5 flex flex-col gap-2 border border-white/10 bg-black/60 backdrop-blur-2xl shadow-2xl focus-within:border-white/30 transition-all">
          ...
          {urlError ? (
            <p className="text-left text-sm text-rose-300">{urlError}</p>
          ) : null}
        ...
        <div className="mt-3 flex justify-end">
          <button
            type="button"
            onClick={handleToggleScanOptions}
            className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-slate-900/80 px-4 py-2 text-xs font-semibold text-slate-200 transition-colors hover:bg-slate-800"
          >
            <span>Scan configuration</span>
            {showScanOptions ? <ChevronUp className="w-3.5 h-3.5 text-slate-300" /> : <ChevronDown className="w-3.5 h-3.5 text-slate-300" />}
          </button>
        </div>
      </div>
